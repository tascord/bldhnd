...
- [x] **Resume queue on service restart.** On startup, all non-terminal downloads are re-driven through their backend with stored uri/info_hash. Verified: killed mid-torrent, restarted, completed. (Also fixed: id counter off-by-one overwriting rows, `list_downloads` infinite-looping on an empty table, librqbit needing `overwrite: true` to resume its own partial files, silent thread panics now captured + logged.)
- [x] **Retry failed downloads.** Enter on a failed/cancelled row re-drives it via new `RetryDownload` IPC; active rows cancel contextually.
