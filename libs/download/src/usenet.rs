...
impl SearchBackend for UsenetSearcher {
    ...
    where
        Self: Sized,
    {
        // Prefer the configured NZB indexer; fall back to bh_server_url.
        config
            .usenet_indexer
            .as_ref()
...
impl DownloadBackend for UsenetDownloader {
    ...
    where
        Self: Sized,
    {
        // Prefer the configured SABnzbd endpoint; fall back to bh_server_url.
        let (url, key) = match &config.sabnzbd {
            Some((url, key)) => (url.clone(), key.clone()),
            None => (config.bh_server_url.clone().unwrap_or_else(|| "http://localhost:8080".into()), None),
        };
        Some(Arc::new(Self::new(&url, key, None, None)))
    }
}
