//! Non-interactive dump of the LayoutView for visual comparison.
//!
//! Usage: cargo run --example layout_dump > /tmp/boba_layout_output.txt
...
use {
    boba::{
        components::{
            ...
            style::{BobaStyle, blend_1d, blend_2d, hex_color, light_dark},
        },
...
fn build_document() -> Surface {
    // Note: not using `has_dark_background()` because stdout is not a TTY here.
    let has_dark_bg = true;
    ...
    // ── Title ──
    let title_style = BobaStyle::new().margin_left(1).margin_right(5).padding(0, 1, 0, 1).italic().fg(hex_color("#FFF7DB"));
    let title_colors =
    ...
    let history_a = history_style
        ...
        .render("The Romans learned from the Greeks that quinces slowly cooked with honey would set when cool.");
    let history_b = history_style.clone().align(Alignment::Center, Position::Top).render(
        "Medieval quince preserves, which went by the French name cotignac, produced in a clear version and a fruit pulp \
         version.",
    );
    let history_c = history_style
        .align(Alignment::Left, Position::Top)
        .margin_right(0)
        .render("In 1524, Henry VIII, King of England, received a box of marmalade from Mr. Hull of Exeter.");
...
fn main() {
    ...
    let mut terminal = Terminal::new(TestBackend::new(100, 56)).unwrap();
    ...
    terminal
        .draw(|f| {
            ...
            let comp = Compositor::new(vec![
                ...
                CompositorLayer::new(modal).x((doc_x + 58) as u16).y((doc_y + 44) as u16),
            ]);
