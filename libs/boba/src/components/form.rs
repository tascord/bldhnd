...
impl FormField {
    ...
    pub fn on_key(&self, code: KeyCode) {
        match self {
            FormField::Input(i) => i.on_key(code),
...
impl Form {
    ...
    pub fn field(mut self, _label: impl Display, input: Input) -> Self {
        self.fields.push(FormField::Input(input));
        self
    }
    ...
    pub fn render_to_buf(&self, area: Rect, buf: &mut Buffer, theme: &crate::theme::Theme) {
        ...
        for (i, field) in self.fields.iter().enumerate() {
            ...
            match field {
                FormField::Input(input) => input.render_to_buf(rect, buf, theme),
