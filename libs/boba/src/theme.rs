...
impl ThemeBuilder {
    ...
    pub fn primary(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.primary = Hsl::new(h, s, l); self }
    pub fn secondary(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.secondary = Hsl::new(h, s, l); self }
    pub fn accent(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.accent = Hsl::new(h, s, l); self }
    pub fn destructive(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.destructive = Hsl::new(h, s, l); self }
    pub fn success(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.success = Hsl::new(h, s, l); self }
    pub fn warning(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.warning = Hsl::new(h, s, l); self }
    pub fn info(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.info = Hsl::new(h, s, l); self }
    pub fn fg_base(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.fg_base = Hsl::new(h, s, l); self }
    pub fn fg_subtle(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.fg_subtle = Hsl::new(h, s, l); self }
    pub fn fg_muted(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.fg_muted = Hsl::new(h, s, l); self }
    pub fn bg_base(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.bg_base = Hsl::new(h, s, l); self }
    pub fn bg_elevated(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.bg_elevated = Hsl::new(h, s, l); self }
    pub fn bg_overlay(mut self, h: f64, s: f64, l: f64) -> Self { self.palette.bg_overlay = Hsl::new(h, s, l); self }

    pub fn primary_color(mut self, c: Color) -> Self { self.palette.primary = Hsl::from_rgb(&c); self }
    pub fn secondary_color(mut self, c: Color) -> Self { self.palette.secondary = Hsl::from_rgb(&c); self }
    pub fn accent_color(mut self, c: Color) -> Self { self.palette.accent = Hsl::from_rgb(&c); self }
    pub fn destructive_color(mut self, c: Color) -> Self { self.palette.destructive = Hsl::from_rgb(&c); self }
    pub fn success_color(mut self, c: Color) -> Self { self.palette.success = Hsl::from_rgb(&c); self }
    pub fn warning_color(mut self, c: Color) -> Self { self.palette.warning = Hsl::from_rgb(&c); self }
    pub fn info_color(mut self, c: Color) -> Self { self.palette.info = Hsl::from_rgb(&c); self }
    pub fn fg_base_color(mut self, c: Color) -> Self { self.palette.fg_base = Hsl::from_rgb(&c); self }
    pub fn fg_subtle_color(mut self, c: Color) -> Self { self.palette.fg_subtle = Hsl::from_rgb(&c); self }
    pub fn fg_muted_color(mut self, c: Color) -> Self { self.palette.fg_muted = Hsl::from_rgb(&c); self }
    pub fn bg_base_color(mut self, c: Color) -> Self { self.palette.bg_base = Hsl::from_rgb(&c); self }
    pub fn bg_elevated_color(mut self, c: Color) -> Self { self.palette.bg_elevated = Hsl::from_rgb(&c); self }
    pub fn bg_overlay_color(mut self, c: Color) -> Self { self.palette.bg_overlay = Hsl::from_rgb(&c); self }
...
impl ImagePaletteExtractor {
    ...
    pub fn quality(mut self, q: u32) -> Self { self.quality = q.max(1); self }
    ...
    pub fn extract(&self, img_path: &str) -> anyhow::Result<Palette> {
        ...
        let bg_colors: Vec<_> = palette.iter().filter(|c| {
            let lum = 0.299 * c[0] + 0.587 * c[1] + 0.114 * c[2];
            lum < 0.4
        }).cloned().collect();

        let fg_colors: Vec<_> = palette.iter().filter(|c| {
            let lum = 0.299 * c[0] + 0.587 * c[1] + 0.114 * c[2];
            lum > 0.6
        }).cloned().collect();

        let accent_colors: Vec<_> = palette.iter().filter(|c| {
            let lum = 0.299 * c[0] + 0.587 * c[1] + 0.114 * c[2];
            lum > 0.4 && lum < 0.6
        }).cloned().collect();
        ...
        Ok(Palette {
            ...
            secondary: Self::rgb_to_hsl(palette[3 % self.max_colors][0], palette[3 % self.max_colors][1], palette[3 % self.max_colors][2]),
