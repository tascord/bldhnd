...
impl List {
    ...
    /// Move the selection by `delta` items without requiring focus.
    pub fn move_selection(&self, delta: i32) {
        let len = self.items.lock_ref().len();
        if len == 0 {
            return;
        }
        let next = (self.selection.get() as i32 + delta).clamp(0, len as i32 - 1);
        self.selection.set(next as usize);
    }
