...
impl Tabs {
    ...
    /// Cumulative end column of each tab as rendered by `render_to_buf`
    /// (each label contributes `"│ {label} "` = chars + 3 columns, starting at
    /// the area's left edge — the horizontal block has no side borders).
    fn tab_ends(&self) -> Vec<usize> {
        let mut ends = Vec::with_capacity(self.labels.len());
        let mut acc = 0usize;
        for label in &self.labels {
            acc += label.chars().count() + 3;
            ends.push(acc);
        }
        ends
    }
    ...
    pub fn on_mouse(&self, area: Rect, ev: &MouseEvent) {
        let ends = self.tab_ends();
        let total_w = ends.last().copied().unwrap_or(0);

        let in_bar =
            ev.column >= area.left() && ev.column < area.right() && ev.row >= area.top() && ev.row < area.bottom();
        let rel_x = ev.column.saturating_sub(area.left()) as usize;
        // Clicks must land on a rendered label, not just anywhere in the bar.
        let on_tab = in_bar && rel_x < total_w;
        ...
        let hovered_idx = ends.iter().position(|&end| rel_x < end).unwrap_or(self.labels.len().saturating_sub(1));
    ...
    pub fn render_to_buf(&self, area: Rect, buf: &mut Buffer, theme: &Theme) {
        ...
        let items: Vec<Span> = self
            ...
            .map(|(i, label)| {
                ...
                let style = match state {
                    TabState::Disabled => ratatui::style::Style::default().fg(muted),
                    TabState::Hovered => {
                    ...
                    TabState::Default => ratatui::style::Style::default().fg(fg),
                };
        ...
        Paragraph::new(Line::from(items)).style(ratatui::style::Style::default().fg(fg)).block(block.into()).render(area, buf);
    }
...
impl Component for Tabs {
    ...
    fn handle_event(&mut self, area: Rect, ev: &Event) { Tabs::handle_event(self, area, ev) }
