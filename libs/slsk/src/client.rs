...
use crate::peer::transfer::{ProgressFn, download_file};
...
impl Default for Config {
    fn default() -> Self {
        Config { server_addr: "server.slsknet.org:2416".into(), listen_port: 2234 }
    }
}
...
impl SlskClient {
    ...
    pub async fn login(&mut self, username: &str, password: &str) -> Result<String> {
        ...
        let msg =
            MsgBuilder::server(ServerCode::Login as u32).str(username).str(password).u32(160).str(&md5_hash).u32(1).build();
        ...
        self.conn.send(&MsgBuilder::server(ServerCode::HaveNoParent as u32).u8(1).build()).await?;
    ...
    pub async fn search(&mut self, query: &str, timeout: Duration) -> Result<Vec<SearchHit>> {
        ...
        let msg = MsgBuilder::server(ServerCode::FileSearch as u32).u32(token).str(query).build();
        self.conn.send(&msg).await?;
    ...
    pub async fn download(&mut self, hit: &SearchHit, dest_dir: &Path, progress: Option<ProgressFn>) -> Result<PathBuf> {
        // Derive a local filename from the last path component.
        let basename = hit.filename.replace('\\', "/").rsplit('/').next().unwrap_or("download").to_owned();
        ...
        let token = self.token();
        let peer = self.connect_peer(addr, &hit.username, "F", token, None).await?;
    ...
    pub async fn download_all(&mut self, hits: &[SearchHit], dest_dir: &Path) -> Vec<Result<PathBuf>> {
    ...
    pub async fn resolve_peer(&mut self, username: &str) -> Result<SocketAddr> {
        let msg = MsgBuilder::server(ServerCode::GetPeerAddress as u32).str(username).build();
        self.conn.send(&msg).await?;
    ...
    pub async fn connect_peer(
        ...
        match PeerConn::connect_and_init(addr, &self.username, conn_type, token, server_token).await {
        ...
        loop {
            ...
            match tokio::time::timeout(remaining, self.conn.recv()).await {
                Ok(Ok(ServerMsg::ConnectToPeer { username: u, conn_type: ct, ip, port, token: st, .. }))
                    if u == username && ct == conn_type =>
                {
                    let peer_addr = SocketAddr::new(ip.into(), port);
                    return PeerConn::connect_and_init(peer_addr, &self.username, conn_type, token, Some(st)).await;
                }
    ...
    pub async fn ping(&mut self) -> Result<()> {
        self.conn.send(&MsgBuilder::server(ServerCode::Ping as u32).build()).await
    }

    /// Request the full room list.
    pub async fn room_list(&mut self) -> Result<Vec<(String, u32)>> {
        self.conn.send(&MsgBuilder::server(ServerCode::RoomList as u32).build()).await?;
    ...
    async fn send_set_wait_port(&mut self, port: u16) -> Result<()> {
        self.conn.send(&MsgBuilder::server(ServerCode::SetWaitPort as u32).u32(port as u32).build()).await
    }

    async fn send_status(&mut self, status: u32) -> Result<()> {
        self.conn.send(&MsgBuilder::server(ServerCode::SetStatus as u32).u32(status).build()).await
    }

    async fn send_shared_counts(&mut self, folders: u32, files: u32) -> Result<()> {
        self.conn.send(&MsgBuilder::server(ServerCode::SharedFoldersFiles as u32).u32(folders).u32(files).build()).await
    }
}
